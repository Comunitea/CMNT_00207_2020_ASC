...
class StockMove(models.Model):
    ...
    def _action_done(self):
        ...
        for move in self:
            if move.sale_line_id.deposit and move.picking_type_id.code == "outgoing":
                formatted_date = datetime.strptime(
                    time.strftime("%Y-%m-%d"), "%Y-%m-%d"
                )
                return_date = datetime.strftime(
                    formatted_date + timedelta(days=15), "%Y-%m-%d"
                )
                values = {
                    "move_id": move.id,
                    "delivery_date": time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    "return_date": move.sale_line_id.deposit_date or return_date,
                    "user_id": move.sale_line_id.order_id.user_id.id,
                    "state": "draft",
                }
...
class StockRule(models.Model):
    ...
    def _push_prepare_move_copy_values(self, move_to_copy, new_date):
        ...
        if (
            self.location_src_id.deposit_location
            and move_to_copy.partner_id.property_deposit_days
        ):
            new_date = fields.Datetime.to_string(
                move_to_copy.date_expected
                + relativedelta(days=move_to_copy.partner_id.property_deposit_days)
            )
        ...
        if sale_line_id:
            ...
            if sale_line_id.deposit:
                ...
                vals[
                    ...
                    sale_line_id.order_id.partner_id.commercial_partner_id.property_stock_deposit.id
                )
                if picking_type_id:
