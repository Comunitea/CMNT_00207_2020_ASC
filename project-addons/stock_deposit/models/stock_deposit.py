...
class StockDeposit(models.Model):
    ...
    product_id = fields.Many2one(
        string="Product", related="move_id.product_id", store=True, readonly=True
    )
    product_uom_qty = fields.Float(
        "Product qty", related="move_id.product_uom_qty", store=True, readonly=True
    )
    product_uom = fields.Many2one(
        related="move_id.product_uom", string="Uom", store=True, readonly=True
    )
    ...
    picking_id = fields.Many2one(
        related="move_id.picking_id", string="Picking", store=True, readonly=True
    )
    ...
    sale_picking_id = fields.Many2one(
        related="sale_move_id.picking_id", string="Sale picking", readonly=True
    )
    ...
    loss_picking_id = fields.Many2one(
        related="loss_move_id.picking_id", string="Loss picking", readonly=True
    )
    ...
    def sale(self):
        ...
        for deposit in self:
            ...
            deposit.move_id.sale_line_id.write(
                {"qty_invoiced": 0, "invoice_status": "to invoice"}
            )
    ...
    def _prepare_deposit_move(self, picking, group):
        ...
        move_template = {
            ...
            "origin": False,
            "route_ids": picking.picking_type_id.warehouse_id
            and [(6, 0, [x.id for x in picking.picking_type_id.warehouse_id.route_ids])]
            or [],
