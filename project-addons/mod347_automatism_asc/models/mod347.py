    ...
    def autogenerate_model347(self):
        ...
        for company in self.env["res.company"].search([]):
            if company.vat and company.phone:
                ...
                unnotified_partners = model347.partner_record_ids.filtered(
                    lambda x: not x.partner_id.date_alert
                    or x.partner_id.date_alert < date_start
                )

                for partner in unnotified_partners:
                    body = _(
                        "The payments amount for the partner {} is currently surpassing 6000€.".format(
                ...
                raise ValidationError(
                    _(
                        "The company {} needs to have a valid VAT and phone.".format(
                            company.name
                        )
                    )
                )
