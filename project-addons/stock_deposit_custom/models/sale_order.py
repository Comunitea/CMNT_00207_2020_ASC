...
class SaleOrderLine(models.Model):
    ...
    qty_in_deposit = fields.Float(string='Quantity in deposit', compute='_compute_qty_in_deposit', store=True,
                                   digits=dp.get_precision('Product Unit of Measure'),
                                   required=True, default=0.0)
    advise_deposit_mail = fields.Boolean('Advise deposit mail', default=False)
    deposit_move_ids = fields.One2many('stock.move', 'deposit_line_id', string='Stock Moves')

    @api.onchange('route_id')
    def _onchange_route_id_set_deposit(self):
        self.deposit = self.route_id and self.route_id.rule_ids and self.route_id.rule_ids[0].location_id.deposit_location
...
class SaleOrder(models.Model):
    ...
    deposit_count = fields.Integer(string='# of Deposits', compute='_compute_deposit_count')
    ...
    def send_advise_email(self):
        ...
        so_ids = self.env['sale.order.line'].search([('deposit', '=', True),
        ...
        template = self.env.ref('stock_deposit_custom.stock_deposit_advise_partner', False)
        for deposit in deposit_ids:
            ...
            composer_id = self.env['mail.compose.message'].with_context(
                ctx).create({})
            values = composer_id.onchange_template_id(template.id, 'comment', deposit.name, deposit.id)['value']
