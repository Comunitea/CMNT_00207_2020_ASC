...
class StockMove(models.Model):
    ...
    move_line_lot_ids = fields.Many2many('stock.production.lot', string="Lots", compute='_compute_move_line_lot_ids')
    proposed_lot_ids = fields.Many2many('stock.production.lot', string="Lots", help="Lots/serial for moves when assign move")
    ...
    def apply_proposed_lots(self):
        if not self:
            return
        move_ids = self.filtered(lambda x: x.picking_type_id.code == 'incoming' and x.picking_type_id.use_proposed_lots and x.proposed_lot_ids and x.state in ['assigned', 'confirmed', 'partially_available'])
        if not move_ids:
            return
        ...
        for move in move_ids:
            if move.location_id.should_bypass_reservation() and  move.product_id.tracking == 'serial' and move.picking_type_id.use_proposed_lots:
                rounding = roundings[move]
                lots = self.env['stock.production.lot'].browse(move.proposed_lot_ids.ids)
                for sml in move.move_line_ids.filtered(lambda x: not x.lot_name and not x.lot_id):
                    if lots:
                ...
                if lots:
                    for lot in lots:
                        move_line_vals_list= dict(move._prepare_move_line_vals(quantity=1), qty_done=1, lot_name=lot.name)
                ...
                elif float_is_zero(move.product_uom_qty - move.quantity_done, precision_rounding=rounding) or move.quantity_done > move.product_uom_qty:
                    assigned_moves |= move
                else:
    ...
    def _action_assign(self):
        ...
        move_ids = self.filtered(lambda x:
                                 x.picking_type_id.code != 'incoming' and
                                 x.picking_type_id.use_proposed_lots and
                                 x.proposed_lot_ids and x.state in ['assigned', 'confirmed', 'partially_available'])
        if move_ids:
    ...
    def apply_lot_list(self, list_lot_ids=[]):
        ...
        if not list_lot_ids:
            list_lot_ids = ['1001', '1002', '1003', '1004', '1005']
        move_lot_names = self.move_line_lot_ids.mapped('name') + self.move_line_ids.mapped('lot_name')
        ...
        if list_lot_ids:
            ...
            for name in list_lot_ids:
                lot_domain = [('product_id', '=', self.product_id.id), ('name', '=', name)]
    ...
    def check_move_line_ids(self):
        if self.state in ('confirmed', 'assigned') and self.picking_type_id.code == 'incoming':
            self.move_line_ids.unlink()
            for lot in self.proposed_lot_ids:
                move_line = self.env['stock.move.line'].create(
                    dict(self._prepare_move_line_vals(quantity=1), qty_done=1, lot_name=lot.name))
    ...
    def get_new_splot_ids(self):
        self.ensure_one()
        action = self.env.ref(
            'stock_move_lot_list.view_new_splot_wzd_act_window').read()[0]
