...
class ProductTemplate(models.Model):
    ...
    def _compute_product_template_sale_alarm(self):
        ...
        for template in self:
            template.days_with_sales = any(x.days_with_sales for x in template.product_variant_ids)
...
class ProductProduct(models.Model):
    ...
    def _compute_product_sales(self):
        ...
        count = dict((item['key'], int(item['value'])) for item in self.env['ir.config_parameter'].search_read(domain, ['key', 'value']))
        domain = [('key', '=', 'field_to_count')]
        field_to_count = self.env['ir.config_parameter'].search_read(domain, ['key', 'value'])
        if field_to_count:
        ...
        for i in range(0,4):
            ...
            res[str] = dict((item['product_id'][0], item[field_to_count]) for item in
                   self.env['sale.order.line'].read_group(domain, ['product_id', field_to_count], ['product_id']))
    ...
    def _compute_product_sale_alarm(self):
        ...
        for product in self:
            days = product.days_for_alarm.days or product.product_tmpl_id.days_for_alarm.days
