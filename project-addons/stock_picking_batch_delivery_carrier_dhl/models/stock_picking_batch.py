...
class StockBatchPicking(models.Model):
    ...
    def compute_resquest_date(self):
        for delivery in self:
            calculated_picking_time = '{:%H:%M:%S}'.format(datetime.now() + timedelta(hours=1))
            picking_time = time.strftime("%Y-%m-%dT{}%Z".format(calculated_picking_time), time.gmtime())
    ...
    def send_shipping(self):
        if self.carrier_id.code == 'DHL':
            ...
            if self.carrier_id.account_id.test_enviroment:
                url = "{}ShipmentRequest".format(self.carrier_id.account_id.service_test_url)
            else:
            ...
            payload = {
                "ShipmentRequest": {
                    "RequestedShipment": {
                        "ShipmentInfo": {
                            "DropOffType": "REGULAR_PICKUP" if self.carrier_id.account_id.daily_picking else "REQUEST_COURIER",
            ...
            try:
                ...
                if r.json()['ShipmentResponse']:
                    ...
                    self.tracking_code = response['ShipmentIdentificationNumber']
                    ...
                    if response['PackagesResult']:
                        ...
                        for package in response['PackagesResult']['PackageResult']:
                            shipment_reference += "{} ".format(package['TrackingNumber'])
                        self.write({
...
class StockPicking(models.Model):
    ...
    def check_shipment_status(self):
        if self.carrier_id.code == 'DHL':
            ...
            if self.carrier_id.account_id.test_enviroment:
                url = "{}TrackingRequest".format(self.carrier_id.account_id.service_test_url)
            else:
                ...
                "trackShipmentRequest": {
                    "trackingRequest": {
                        "TrackingRequest": {
                            "Request": {
                                "ServiceHeader": {
                                    "MessageTime": "{}".format(self.batch_id.request_date.replace('GMT', '')),
                                    "MessageReference": "{}".format(self.get_message_reference(32)) #'212f797bf00a4afa9654cea8449c9779'
                                }
                            },
                            "ReferenceQuery": {
            ...
            if 'ShipmentInfo' in awbinfoitem:
                delivery_status = shipment_info['ShipmentEvent']['ArrayOfShipmentEventItem']['ServiceEvent']['EventCode']
