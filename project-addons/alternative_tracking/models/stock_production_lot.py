...
class StockProductionLot(models.Model):
    ...
    def _compute_tracking_moves(self):
        for lot in self:
            domain = [("state", "=", "done"), '|', ('lot_id', '=', lot.id), ("serial_ids", "=", lot.id)]
    ...
    def update_real_location_id(self):
        if not self:
            self = self.search([])
        for lot in self.sudo().filtered(lambda x: x.virtual_tracking):
            _logger.info("Actualizando {}".format(lot.name))
            real_location_id = lot.real_location_id
            ...
            _logger.info(">>>> De {} ({}) a {}".format(real_location_id.display_name, real_location_id.serial_location.display_name, lot.real_location_id.display_name))

    @api.multi
    @api.onchange('real_location_id')
    def onchange_real_loc_id(self):
        for serial in self:
            if serial.real_location_id:
                serial.location_id = serial.real_location_id.serial_location or serial.real_location_id
