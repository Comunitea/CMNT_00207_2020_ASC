...
class SaleOrder(models.Model):
    ...
    def action_view_batch_picking(self):
        ...
        action = self.env.ref('stock_picking_batch_extended.action_stock_batch_picking_tree').read()[0]
        if len(batches) > 1:
            action['domain'] = [('id', 'in', batches.ids)]
        elif len(batches) == 1:
            form_view = [(self.env.ref('stock_picking_batch_extended.stock_batch_picking_form').id, 'form')]
            if 'views' in action:
