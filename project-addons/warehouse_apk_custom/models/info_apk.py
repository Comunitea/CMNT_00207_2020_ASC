...
class InfoApk(models.AbstractModel):
    ...
    def get_apk_product(self, code):
        ...
        product_id = (
            ...
            .filtered(lambda x: x.default_on)
        )
        if len(product_id) > 1:
            raise ValueError(
                "Se han encontrado varios productos para este código {}".format(code)
            )
        return product_id
    ...
    def m2o_dict(self, field):
        if field:
            return {
                "id": field.id,
                "name": field.name,
                "wh_code": field.wh_code or field.name[:1],
            }
        else:
            return {"id": False}
    ...
    def m2o_dict(self, field):
        if field:
            return {
                "id": field.id,
                "name": field.name,
                "wh_code": field.wh_code or field.code and field.code[:1],
            }
        else:
            return {"id": False}
